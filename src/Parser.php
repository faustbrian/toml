...
use Cline\Toml\Exception\InvalidUtf8InputException;
use Cline\Toml\Exception\UnexpectedEndOfInputException;
...
class Parser extends AbstractParser
{
    ...
    public function parse(string $input): array
    {
        if (preg_match('//u', $input) === false) {
            throw InvalidUtf8InputException::create();
        }
    ...
    private function processExpression(TokenStream $ts): void
    {
        if ($ts->isNext('T_HASH')) {
            ...
            $this->unexpectedTokenError($this->requireToken($ts), $msg);
        }
    }
    ...
    /**
     * @return object{value: mixed, type: string} An object with two public properties: value and type.
     */
    private function parseSimpleValue(TokenStream $ts): object
    {
        if ($ts->isNext('T_BOOLEAN')) {
            ...
            $this->unexpectedTokenError(
                $this->requireToken($ts),
                'Expected boolean, integer, long, string or datetime.'
            );
        }

        return (object) ['value' => $value, 'type' => $type];
    }
    ...
    private function parseBasicString(TokenStream $ts): string
    {
        ...
        while (! $ts->isNext('T_QUOTATION_MARK')) {
            if ($ts->isNextAny(self::$tokensNotAllowedInBasicStrings)) {
                $this->unexpectedTokenError($this->requireToken($ts), 'This character is not valid.');
            }

            $value = $ts->isNext('T_ESCAPED_CHARACTER') ? $this->parseEscapedCharacter($ts) : $this->requireToken($ts)->getValue();
            $result .= $value;
        }
    ...
    private function parseMultilineBasicString(TokenStream $ts): string
    {
        ...
        while (! $ts->isNext('T_3_QUOTATION_MARK')) {
            if ($ts->isNext('T_EOS')) {
                $this->unexpectedTokenError($this->requireToken($ts), 'Expected token "T_3_QUOTATION_MARK".');
            }
    ...
    private function parseLiteralString(TokenStream $ts): string
    {
        ...
        while (! $ts->isNext('T_APOSTROPHE')) {
            ...
            $result .= $this->requireToken($ts)->getValue();
        }
    ...
    private function parseMultilineLiteralString(TokenStream $ts): string
    {
        ...
        while (! $ts->isNext('T_3_APOSTROPHE')) {
            if ($ts->isNext('T_EOS')) {
                $this->unexpectedTokenError($this->requireToken($ts), 'Expected token "T_3_APOSTROPHE".');
            }
        ...
        while (! $ts->isNext('T_RIGHT_SQUARE_BRAKET')) {
            ...
            if ($ts->isNext('T_LEFT_SQUARE_BRAKET')) {
                ...
                if ($valueStruct->type !== $leaderType) {
                    $valueForError = $valueStruct->value;
                    if ($valueForError instanceof \DateTime) {
                        $valueForError = $valueForError->format('Y-m-d\TH:i:s');
                    }
                    $this->syntaxError(sprintf(
                        'Data types cannot be mixed in an array. Value: "%s".',
                        is_scalar($valueForError) ? (string) $valueForError : gettype($valueForError)
                    ));
                }
    ...
    private function matchNext(string $tokenName, TokenStream $ts): string
    {
        if (! $ts->isNext($tokenName)) {
            $this->unexpectedTokenError($this->requireToken($ts), "Expected \"{$tokenName}\".");
        }

        return $this->requireToken($ts)->getValue();
    }

    private function requireToken(TokenStream $ts): Token
    {
        $token = $ts->moveNext();
        if ($token === null) {
            throw UnexpectedEndOfInputException::create();
        }
    ...
    private function errorIfNextIsNotNewlineOrEOS(TokenStream $ts): void
    {
        if (! $ts->isNextAny(['T_NEWLINE', 'T_EOS'])) {
            $this->unexpectedTokenError($this->requireToken($ts), 'Expected T_NEWLINE or T_EOS.');
        }
    }
